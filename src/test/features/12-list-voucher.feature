...
Feature: Retrieve the List of Vouchers
  ...
  Scenario Outline: List of voucher types
    When the operator requires the vouchers with type '<typeId>', status '<status>', userId '<userId>', merchantId '<merchantId>' and transactionId '<transactionId>'
    Then the user retrieve the list with '<result>' vouchers
    Examples:
      | typeId | status    | userId | transactionId | merchantId | result                                                      |
  ...
  Scenario Outline: Voucher in error
    When the operator requires the vouchers with type '<typeId>', status '<status>', userId '', merchantId '' and transactionId ''
    Then the operator receive the error code '<errorCode>' and description '<errorDescription>'
    Examples:
      | typeId | status | errorCode      | errorDescription             |
